...
int main() {
  ...
  auto [TNSol_F, TNSol] = MakeAtlasField("TNSol", mesh.nodes().size(), k_size);   //analytical solution
  auto [TC_F, TC] = MakeAtlasField("TC", mesh.cells().size(), k_size);  
  
  auto [TE_F, TE] = MakeAtlasField("TE", mesh.edges().size(), k_size);
  auto [TEinit_F, TEinit] = MakeAtlasField("TEinit", mesh.edges().size(), k_size);
  auto [TE_t_F, TE_t] = MakeAtlasField("TEinit", mesh.edges().size(), k_size);
  auto [TEnabla2_F, TEnabla2] = MakeAtlasField("TEnabla2", mesh.edges().size(), k_size);

  // Geometrical factors on edges  
  auto [inv_L_F, inv_L] = MakeAtlasField("invL", mesh.edges().size(), k_size);       // inverted edge length  
  auto [inv_vvL_F, inv_vvL] = MakeAtlasField("invvvL", mesh.edges().size(), k_size); // inverted vert vert length    

  // Geometrical factors on vertices
  auto [nnbhV_F, nnbhV] = MakeAtlasField("nnbh", mesh.nodes().size(), k_size);        // number of edge neighbors
                                                                                      // for simple averaging

  // Masks for boundary conditions     
  auto [boundary_edges_F, boundary_edges] = MakeAtlasField("boundary_edges", mesh.edges().size(), k_size);   
  auto [boundary_cells_F, boundary_cells] = MakeAtlasField("boundary_cells", mesh.edges().size(), k_size);     

  // These should be globals (currently emulated with fields)
  auto [kappa_Field_F, kappa_Field] = MakeAtlasField("kappa_Field", mesh.edges().size(), k_size);
  auto [dt_Field_F, dt_Field] = MakeAtlasField("dt_Field", mesh.edges().size(), k_size);
  ...
  for(int edgeIdx = 0; edgeIdx < mesh.edges().size(); edgeIdx++) {
    ...
    inv_vvL(edgeIdx, level) = (vert_vert_length == 0) ? 0 : 1. / vert_vert_length;
  }  
  ...
  //===------------------------------------------------------------------------------------------===//
   ...
   while(t < t_final) {    
    ...
    dawn_generated::cxxnaiveico::diffusion<atlasInterface::atlasTag>(mesh, k_size, 
                                                                TN, 
                                                                TE, TEinit, TE_t, TEnabla2, 
                                                                inv_L, inv_vvL, nnbhV,
                                                                boundary_edges,
                                                                kappa_Field, dt_Field).run();
    ...
    if(step % outputFreq == 0) {
      ...
      dumpNodeField(buf, mesh, TN, level);
    }
